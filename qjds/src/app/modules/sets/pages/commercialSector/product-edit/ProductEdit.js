...
export function ProductEdit({
  ...
  const { actionsLoading, productForEdit } = useSelector(
    (state) => ({
      actionsLoading: state.commercialSectors.actionsLoading,
      productForEdit: state.commercialSectors.districtForEdit,
    }),
    shallowEqual
  );

  useEffect(() => {
    dispatch(actions.fetchCommercialSectorByID(id));
  }, [id, dispatch]);

  useEffect(() => {
    let _title = id ? "Edit Commercial Sector Details" : "New Commercial Sector";
  ...
  const saveProduct = (values) => {
    if (!id) {
      setLoader(true)
      dispatch(actions.createCommercialSector(values)).then(
        (res) => {
          setLoader(false)
          setisSuccess({
            success: 2,
            message: "Commercial Sector created successfully",
          });
          setTimeout(() => {
      ...
      dispatch(
        ...
        (res) => {
          ...
          setisSuccess({
            success: 2,
            message: "Commercial Sector updated successfully",
          });
          setTimeout(() => {
            setisSuccess({ success: 0, message: "" });
            backToProductsList();
