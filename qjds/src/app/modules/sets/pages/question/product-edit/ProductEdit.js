...
export function ProductEdit({
  ...
  const { actionsLoading, productForEdit } = useSelector(
    (state) => ({
      ...
      productForEdit: state.questions.districtForEdit,
    }),
    shallowEqual
  );
  ...
  useEffect(() => {
    let _title = id ? "Edit Question Details" : "New Question";
  ...
  const saveProduct = (values) => {
    if (!id) {
      setLoader(true)
      dispatch(actions.createQuestion(values)).then(
        (res) => {
          setLoader(false)
          setisSuccess({
            success: 2,
