...
export function ProductEdit({
  ...
  const { actionsLoading, productForEdit } = useSelector(
    (state) => ({
      ...
      productForEdit: state.terms_conditions.terms_conditionForEdit,
    }),
    shallowEqual
  );

  useEffect(() => {
    dispatch(actions.fetchterms_condition(id));
  }, [id, dispatch]);

  useEffect(() => {
    let _title = id ? "" : "New Term";
    if (productForEdit && id) {
      _title = `Edit Term '${productForEdit.terms_conditions_name}'`;
    }
  ...
  const saveProduct = (values) => {
    if (!id) {
      setLoader(true)
      dispatch(actions.createterms_condition(values)).then(
        (res) => {
          setLoader(false)
          setisSuccess({
            success: 2,
      ...
      setLoader(true)
      dispatch(actions.updateterms_condition(values)).then(
        (res) => {
          setLoader(false)
          setisSuccess({
            success: 2,
