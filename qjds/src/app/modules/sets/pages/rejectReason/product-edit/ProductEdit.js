...
export function ProductEdit({
  ...
  const { actionsLoading, productForEdit } = useSelector(
    (state) => ({
      actionsLoading: state.rejectReasons.actionsLoading,
      productForEdit: state.rejectReasons.districtForEdit,
    }),
    shallowEqual
  );

  useEffect(() => {
    dispatch(actions.fetchRejectReasonByID(id));
  }, [id, dispatch]);

  useEffect(() => {
    let _title = id ? "Edit Reject Reason Details" : "New Reject Reason";
  ...
  const saveProduct = (values) => {
    if (!id) {
      setLoader(true)
      dispatch(actions.createRejectReason(values)).then(
        (res) => {
          setLoader(false)
          setisSuccess({
            success: 2,
