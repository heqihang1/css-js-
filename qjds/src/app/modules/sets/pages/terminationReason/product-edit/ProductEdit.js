...
export function ProductEdit({
  ...
  const { actionsLoading, productForEdit } = useSelector(
    (state) => ({
      actionsLoading: state.terminationReasons.actionsLoading,
      productForEdit: state.terminationReasons.districtForEdit,
    }),
    shallowEqual
  );

  useEffect(() => {
    dispatch(actions.fetchTerminationReasonByID(id));
  }, [id, dispatch]);

  useEffect(() => {
    let _title = id ? "Edit Termination Reason Details" : "New Termination Reason";
  ...
  const saveProduct = (values) => {
    if (!id) {
      setLoader(true)
      dispatch(actions.createTerminationReason(values)).then(
        (res) => {
          setLoader(false)
          setisSuccess({
            success: 2,
            message: "Termination reason created successfully",
          });
          setTimeout(() => {
      ...
      setLoader(true)
      dispatch(
        actions.updatTerminationReason({
          _id: id,
          ...values,
        })
      ).then(
        (res) => {
          ...
          setisSuccess({
            success: 2,
            message: "Termination reason updated successfully",
          });
          setTimeout(() => {
            setisSuccess({ success: 0, message: "" });
            backToProductsList();
