...
export function ProductActivateDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.terminationReasons.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteProduct = () => {
    dispatch(actions.activateTerminationReason(id)).then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        {!isLoading && <span>Please confirm active this termination reason</span>}
        <br />
        <strong>{query.get("terminationReason")}</strong>
      </Modal.Body>
      <Modal.Footer>
        <div>
          <button
