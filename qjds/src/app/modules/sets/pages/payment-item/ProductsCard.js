...
import { generateColumns } from "../../../../utils/sets/payment-item/tableDeps";
...
export function ProductsCard(props) {
  ...
  const { currentState } = useSelector(
    (state) => ({ currentState: state.paymentMethods }),
    shallowEqual
  );
  // Products Redux state
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(actions.findpaymentMethods(queryParams));
  ...
  return (
    <Card>
      <CardHeader title="Payment Items list">
        <CardHeaderToolbar>
        {checkPermission('PAYMENT_ITEM', 'can_add') && (
          <button
            type="button"
            className="btn btn-primary"
            onClick={() => history.push("/sets/payment-item/new")}
          >
            New Item
          </button>
        )}
