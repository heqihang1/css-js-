...
import { generateColumns } from "../../../../utils/sets/service-item/tableDeps";
...
export function ProductsCard(props) {
  ...
  useEffect(() => {
    dispatch(actions.fetchservices(queryParams));
  ...
  return (
    <Card>
      <CardHeader title="Service Item list">
        <CardHeaderToolbar>
        {checkPermission('SERVICE_ITEM', 'can_add') && (
          <button
            type="button"
            className="btn btn-primary"
            onClick={() => history.push("/sets/service-item/new")}
          >
            New Item
          </button>
        )}
