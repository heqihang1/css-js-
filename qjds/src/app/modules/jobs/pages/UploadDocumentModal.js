...
import { uploadDocumentJob } from "../_redux/job/jobCrud";
import { shallowEqual, useDispatch, useSelector } from "react-redux";
import { Field, Form, Formik } from "formik";
import { DragDropFile } from "../../../components/DragAndDrop";

const UploadDocumentModal = ({ doc, show, onHide, onSuccess, isFinishJobOrder }) => {
  const history = useHistory();
  const dispatch = useDispatch();
  const { isLoading } = useSelector(
    (state) => ({
      isLoading: state.job.actionsLoading,
    }),
    shallowEqual
  );
  ...
  const uploaddocument = async () => {
    ...
    uploadDocumentJob(doc._id, requestBody).then(() => {
      onHide();
      onSuccess();
    });
  };
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">
          {(isFinishJobOrder) ? 'Finish Job Order' : 'Upload Signed Job Order' }
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Formik
              ...
              <Form className="form form-label-right" onDragEnter={handleDrag}>
                <div className="col form-group">
                  <div className="form-group ">
                    <div>{ isFinishJobOrder ? 'Job order singed document (not required)' : '' }</div>
                    <DragDropFile
                      utils={{
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            ...
            disabled={!isFinishJobOrder && filesSelected.length === 0}
          >
            { isFinishJobOrder ? 'Finish' : 'Confirm' }
          </button>
        </div>
      </Modal.Footer>
    </Modal>
