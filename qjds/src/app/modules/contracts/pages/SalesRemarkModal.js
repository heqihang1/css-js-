...
import * as actions from "../_redux/contractsSalesRemark/salesRemarkActions";
import { generateColumns } from "../../../utils/contractSalesRemark/tableDeps";
...
const SalesRemarkModal = ({ show, onHide, onShow, quote_id, totalSalesRemark }) => {
  ...
  const { currentState } = useSelector(
    (state) => ({ currentState: state.contractSalesRemark }),
    shallowEqual
  );
  ...
  function addRemark() {
    dispatch(actions.createSalesRemark({ remark: remark, contract_id: quote_id, color: color })).then(() => {
  ...
  const columns = generateColumns(history, checkPermission("CONTRACT_DETAILS_SALES_REMARK", "can_delete"), {
    deleteSalesRemark
  });
  ...
  return (
    <>
      ...
      >
        ...
        <Modal.Body>
          {checkPermission("CONTRACT_DETAILS_SALES_REMARK", "can_add") && (
            <div className="row">
