...
import { createJobOrder } from "../../_redux/contracts/contractsCrud";
...
export default function Actions({
  ...
  const [nextRemindMonthModal, setNextRemindMonthModal] = useState(false);
  ...
  function createJobOrderDialog(item) {
  ...
  useEffect(() => {
    setInterval(() => {
      if (document.getElementById("contract-pdf-component")) {
        setOutput(document.getElementById("contract-pdf-component").innerHTML);
      }
    }, 1000);
  }, []);
  ...
  function getJobOrder() {
    ...
    dispatch(jobActions.fetchJobsByContractQuotationNo(contract_no)).then(
      (res) => {
              ...
              <div className="d-sm-flex flex-sm-row justify-content-end">
                ...
                >
                  <a
                    ...
                    onClick={() => window.open(`/contracts/${item._id}/details?isHistory=true`) }
                  >
  ...
  // const MyDocument = () => (
  //   <Document>
  //     <Page size="A4" style={styles.page} wrap debug={false}>
  //       <View
  //         style={{
  //           display: "flex",
  //           flexDirection: "row",
  ...
  //       >
  //         <Image
  //           style={{ flex: 1, height: "40.56px", width: "18.72px" }}
  //           src="/media/details_logo.png"
  //         />
  //         <Text style={{ flex: 3.2, textAlign: "center", fontSize: "19px" }}>
  //           Agreement
  //         </Text>
  //         <Text style={{ flex: 1.5, textAlign: "right", fontSize: "8px" }}>
  //           {isChinesePDF ? <Text>合約編號</Text> : "Agreement No"}:{" "}
  //           <Text>
  //             #{contract_no}
  //             {"\n"}
  //           </Text>
  ...
  //           {isChinesePDF ? <Text>服務期:</Text> : "Service Period: "}
  //         </Text>
  //         <Text style={{ fontSize: "9px", marginTop: "5px" }}>
  //           {isChinesePDF ? (
  //             <Text>
  //               在 {productForEdit?.contract_period} 個月服務期內雙方協議安排。
  //             </Text>
  //           ) : (
  //             `Arranged by mutual agreement during the ${productForEdit?.contract_period} month service period.`
  //           )}
  //         </Text>
  //         <Text style={{ fontSize: "9px", marginTop: "5px" }}>
  //           {isChinesePDF ? (
  //             <Text>由 _______________ 至 _______________。</Text>
  //           ) : (
  //             "From_______________ to _______________."
  //           )}{" "}
  //           {isChinesePDF ? (
  //             <Text style={{ color: "red" }}>(由客戶填寫)</Text>
  //           ) : (
  ...
  //       >
  //         <Text style={{ flex: 1 }}>
  //           {isChinesePDF ? <Text>專員</Text> : "Customer Officer"}:{" "}
  //           {customer_office_worker}
  //         </Text>
  //         <Text style={{ flex: 1, textAlign: "right" }}>
  //           {isChinesePDF ? <Text>合共</Text> : "Total Amount"}${" "}
  //           {productForEdit?.contract_amount}
  //         </Text>
  //       </View>
  //       <Text
  //         style={{
  ...
  //       {isChinesePDF ? (
  //         <Text
  //           style={styles.contractNoChinese}
  //           render={() => `合約編號: #${contract_no}`}
  //           fixed
  //         />
  //       ) : (
  //         <Text
  //           style={styles.contractNoEnglish}
  //           render={() => `Agreement No.: #${contract_no}`}
  //           fixed
  //         />
  //       )}
  ...
  function getInvoices(IsCheckInvoice) {
    dispatch(actions.fetchInvoicesByContractQuotationNo(contract_no)).then(
      (res) => {
  ...
  useEffect(() => {
    if (contract_no) {
      fetchJobOrders();
      getInvoices(true);
      setNextRemindMonth(productForEdit?.next_remind_month);
    }
    // eslint-disable-next-line
  }, [contract_no]);
  ...
  function generateInvoice() {
    ...
    let data = {
      ...
      address: (places && places.length > 0)
        ...
        : [],
      amount: contract_amount > 0 ? contract_amount.toFixed(2) : "0",
    };
    dispatch(actions.createinvoice(data)).then(
      (res) => {
        ...
        dispatch(
          contractActions.changecontractsStatus(productForEdit._id, {
            status: contractStatuses.PENDING_PAYMENT,
          })
        ).then((res2) => {
          refreshPage();
        });
      ...
      return jobs.map((item) => {
        return (
          <TableRow>
            ...
            <TableCell>
              {item?.working_location?.location_name}
            </TableCell>
            <TableCell>
              ...
              <div className="d-sm-flex flex-sm-row justify-content-end">
                ...
                >
                  <a
                    className="btn btn-icon btn-light btn-hover-primary btn-sm mx-3"
                    onClick={() => {
                      createJobOrderDialog(item);
                      setIsAdditionalJobOrder(false);
                      setIsCloneJobOrder(true)
                    }}
                  >
                    <span className="svg-icon svg-icon-md svg-icon-primary">
  ...
  function saveNextRemindMonth() {
    const requestBody = {
      next_remind_month: nextRemindMonth,
    };

    dispatch(
      contractActions.updateNextRemindMonth(productForEdit._id, requestBody)
    ).then(() => {
      refreshPage();
      setNextRemindMonthModal(false);
    });
  }
    ...
    <>
      ...
      <div className="d-flex flex-column">
          ...
          <CardBody>
            <div className="d-flex flex-column">
              ...
              {productForEdit?.status &&
                (productForEdit?.status === contractStatuses.PENDING ||
                  productForEdit?.status === contractStatuses.REJECTED ||
                  productForEdit?.status === contractStatuses.CLIENT_REJECTED ||
                  productForEdit?.status === contractStatuses.APPROVED || 
                  ((productForEdit?.status === contractStatuses.CONFIRMED || productForEdit?.status === contractStatuses.PENDING_PAYMENT) && checkPermission("SPECIAL_EDIT", "can_edit"))) &&
                checkPermission("CONTRACT", "can_edit") && (
                  <button
                    type="button"
                    className="btn btn-default mb-4 border bg-white border-dark text-dark"
                    onClick={() => {
                      history.push(`/contracts/${productForEdit._id}/edit`);
                    }}
                  >
                    Edit
                  </button>
              ...
              {productForEdit?.status &&
                (productForEdit?.status === contractStatuses.PENDING ||
                  productForEdit?.status === contractStatuses.REJECTED || productForEdit?.status === contractStatuses.CLIENT_REJECTED) &&
                checkPermission("CONTRACT", "can_approval") && (
                  <button
                    type="button"
              ...
              {productForEdit?.status &&
                productForEdit?.status === contractStatuses.SPECIAL_PENDING &&
        ...
        {productForEdit?.status &&
          ...
          (productForEdit?.status === contractStatuses.CONFIRMED ||
            productForEdit?.status === contractStatuses.PENDING_PAYMENT) && (
            <Card>
              <CardBody>
                <div className="d-flex flex-column">
                  <button
        ...
        {productForEdit?.status &&
          checkPermission("CONTRACT", "can_confirm") &&
          (productForEdit?.status === contractStatuses.CONFIRMED ||
            productForEdit?.status === contractStatuses.PENDING_PAYMENT) && (
            <Card>
              <CardBody>
                <div className="d-flex flex-column">
                  <button
                    type="button"
                    className="btn btn-default mb-4 border bg-white border-dark text-dark"
                    onClick={() => setNextRemindMonthModal(true)}
                  >
                    Next Remind Month
                  </button>
                </div>
        ...
        {productForEdit?.status &&
          (productForEdit?.status === contractStatuses.APPROVED || productForEdit?.status === contractStatuses.CONFIRMED || productForEdit?.status === contractStatuses.PENDING_PAYMENT) &&
          checkPermission("CONTRACT", "can_edit") && (
            <Card>
              <CardBody>
                <div className="d-flex flex-column">
        ...
        {/* <button style={{ display: "none" }} type="button">
          <PDFDownloadLink
            ...
            document={<InvoicePDF />}
            fileName={`Invoice-${contract_no}.pdf`}
          >
            {" "}
            PDF
          </PDFDownloadLink>
        ...
        {checkPermission('CONTRACT_DETAILS_SALES_REMARK', 'can_read') && (
          <Card>
            <CardBody>
              <div className="d-flex flex-column">
                <button
                ...
                <>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setIsAdditionalJobOrder(true);
                      setIsCloneJobOrder(false)
                    }}
                  >
      ...
      >
        ...
        <Modal.Footer>
          <div>
            <button
              type="button"
              onClick={() => setNextRemindMonthModal(false)}
              className="btn btn-secondary mr-2"
            >
              Close
            </button>
      ...
      <Card>
        ...
        >
          ...
          <input
            type="hidden"
            value={`${window.location.origin}/pdf/contract`}
            name="url"
          ></input>
          <input type="hidden" value={contract_no} name="contract_no"></input>
