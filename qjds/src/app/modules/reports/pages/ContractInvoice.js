...
export function ContractInvoice() {
  ...
  function genereateInvoice(data) {
    ...
    for (let i = 0; i <= data.length; i++) {
      if (data[i]) {
        if (moment(data[i].date).format("DD") === "01" || data[i].contract_invoice) {
          ws.addRow([
            ...
            (data[i].date && data[i].payment_term_id?.over_due_day) ? moment(data[i].date, "YYYY-MM-DD").add('days', data[i].payment_term_id?.over_due_day).format("DD/MM/YYYY") : (data[i].date) ? moment(data[i].date).format("DD/MM/YYYY") : '',
          ])
          ws.getCell(`E${row_no}`).alignment = { vertical: 'middle', horizontal: 'right' }
          row_no++;
        }
    ...
    wb.xlsx.writeBuffer(`Contract Invoice - ${moment().format('YYYY-MM-DD')}.xlsx`).then((buffer) => {
      saveAs(
        new Blob([buffer], { type: "application/octet-stream" }),
        `Contract Invoice - ${moment().format('YYYY-MM-DD')}.xlsx`
      )
    })
  }
  ...
  function donwloadInvoice(data, loopCount, invoiceData) {
    if (data.length == loopCount) {
      ...
      zip.generateAsync({ type: 'blob' }).then(function (content) {
        FileSaver.saveAs(content, 'ContractInvoice.zip');
      });
      setLoading(false)
    } else {
      if (data[loopCount] && (moment(data[loopCount].date).format("DD") === "01" || data[loopCount].contract_invoice)) {
  ...
  function getInvoices(values) {
    ...
    dispatch(actions.fetchReportInvoices('AJ', values.from, values.to, values?.customer, isDonwload)).then(
      (res) => {
  ...
  const ContractInvoiceSchema = Yup.object().shape({
  ...
  return (
    <>
      <Formik
        ...
        validationSchema={ContractInvoiceSchema}
        initialValues={{
          from: '',
          to: ''
        }}
