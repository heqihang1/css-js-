...
export const ActionsColumnFormatter = () => (
    ...
    row,
    rowIndex,
    { preview, copy, reuse, rescheduleJobOrder, cancelJobOrder, uploadDocument, edit }
) => {
  return ( 
    <div className="d-sm-flex flex-sm-row justify-content-end">
      ...
      {!window.location.pathname.includes("/all") && checkPermission('JOB', 'can_edit') && (row.status === jobStatuses.ARRAGED || row.status === jobStatuses.REJECTED || row.status === jobStatuses.PENDING || row.status === jobStatuses.APPROVED) && (
        <>
          <OverlayTrigger
            overlay={<Tooltip id="products-edit-tooltip">Edit Job Order</Tooltip>}
          >
            <a
              className="btn btn-icon btn-light btn-hover-primary btn-sm mx-1"
      ...
      {window.location.pathname.includes("/arraged-job") && checkPermission('JOB', 'can_edit') && (
          ...
          >
            <a
            className="btn btn-icon btn-light btn-hover-danger btn-sm mx-1"
            onClick={() => uploadDocument(row._id, true)}
          >
            <span className="svg-icon svg-icon-md svg-icon-danger">
              <SVG
        ...
        {window.location.pathname.includes("/finished") && checkPermission('JOB', 'can_edit') && (
          <OverlayTrigger
            overlay={
              <Tooltip id="products-delete-tooltip">
          ...
          >
            <a
              ...
              onClick={() => uploadDocument(row._id, false)}
            >
